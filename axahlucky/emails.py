...
from axahlucky.extensions import mail

def _send_asynv_mail(app, message):
    with app.app_context():
        mail.send(message)

def send_mail(to, subject, template, **kwargs):
    message = Message(current_app.config['AXAHLUCKY_MAIL_SUBJECT_PREFIX']+subject, recipients=[to])
    message.body = render_template(template + '.txt', **kwargs)
    Message.html = render_template(template + '.html', **kwargs)
    app = current_app._get_current_object()
    thr = Thread(target= _send_asynv_mail, args=[app, message])
    thr.start()
    return thr

def send_hello_mail(user=None):
    if not user:
        to = current_app.config['ADMIN_EMAIL_ADDRESS']
    else:
        to = user.email
